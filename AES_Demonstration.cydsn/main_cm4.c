...
#define SCREEN_HEADER "*\t\tCE220465 PSoC 6 Cryptography: AES Demonstration\r\n*\r\n"\
					  "*\tThis code example demonstrates encryption and decryption of"\
					  "data using\r\n*\tthe Advanced Encryption Scheme (AES)algorithm in PSoC 6 MCU."\
	...
	for(;;)
	{
		switch(msgStatus)
		{
			case MESSAGE_ENTER_NEW:
				...
				messagesize = 0;
				printf("\r\nEnter the message:\r\n");
				Cy_SCB_UART_Receive(UART_HW, &message[messagesize], 1, &uartContext);
				msgStatus = MESSAGE_NOT_READY;
				break;

			case MESSAGE_NOT_READY:

				uartStatus = Cy_SCB_UART_GetReceiveStatus(UART_HW, &uartContext);
				if(uartStatus != CY_SCB_UART_RECEIVE_ACTIVE)
				{
					...
					{
						Cy_SCB_UART_Put(UART_HW, message[messagesize]);
						messagesize++;
