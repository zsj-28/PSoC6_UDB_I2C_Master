...
uint8_t buffer_index = 0; // Start at 0
uint8_t UART_buffer[1000] = {0}; // Empty buffer is all 0s, max capacity is 1000
...
void loop() {
    ...
    // Read UART bytes whenever available
    if (Serial1.available()) {
        ...
        if ((buffer_index > 1) && (buffer_index == UART_buffer[1] + 3)) { // short-circuit until after at least 2 bytes are read
            buffer_index = 0;
    ...
    if ((millis() - lastAvailable) > 50) {
        Serial.println("UART buffer timeout");
        buffer_index = 0;
    }
