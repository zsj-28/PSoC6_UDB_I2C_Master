...
volatile uint8_t `$INSTANCE_NAME`_mstrStatus;     /* Master Status byte  */
volatile uint8_t `$INSTANCE_NAME`_mstrControl;    /* Master Control byte */

/* Transmit buffer variables */
volatile uint8_t * `$INSTANCE_NAME`_mstrRdBufPtr;     /* Pointer to Master Read buffer */
volatile uint8_t   `$INSTANCE_NAME`_mstrRdBufSize;    /* Master Read buffer size       */
volatile uint8_t   `$INSTANCE_NAME`_mstrRdBufIndex;   /* Master Read buffer Index      */

/* Receive buffer variables */
volatile uint8_t * `$INSTANCE_NAME`_mstrWrBufPtr;     /* Pointer to Master Write buffer */
volatile uint8_t   `$INSTANCE_NAME`_mstrWrBufSize;    /* Master Write buffer size       */
volatile uint8_t   `$INSTANCE_NAME`_mstrWrBufIndex;   /* Master Write buffer Index      */
      ...
      `=ReentrantKeil($INSTANCE_NAME . "_MasterWriteBuf")`
{
    ...
    if(NULL != wrData)
    {
        ...
        else if(`$INSTANCE_NAME`_SM_MSTR_HALT == `$INSTANCE_NAME`_state)
        {
            ...
            `$INSTANCE_NAME`_mstrStatus &= (uint8_t) ~`$INSTANCE_NAME`_MSTAT_XFER_HALT;
        }
        else
        {
        ...
        if(`$INSTANCE_NAME`_MSTR_NO_ERROR == errStatus)
        {
            ...
            `$INSTANCE_NAME`_mstrWrBufPtr   = (volatile uint8_t *) wrData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            `$INSTANCE_NAME`_mstrControl = mode;

            /* Clear write status history */
            `$INSTANCE_NAME`_mstrStatus &= (uint8_t) ~`$INSTANCE_NAME`_MSTAT_WR_CMPLT;

            /* Hardware actions: write address and generate Start or ReStart */
            `$INSTANCE_NAME`_DATA_REG = (uint8_t) (slaveAddress << `$INSTANCE_NAME`_SLAVE_ADDR_SHIFT);
      ...
      `=ReentrantKeil($INSTANCE_NAME . "_MasterReadBuf")`
{
    ...
    if(NULL != rdData)
    {
        ...
        if(`$INSTANCE_NAME`_MSTR_NO_ERROR == errStatus)
        {
            ...
            `$INSTANCE_NAME`_mstrRdBufPtr    = (volatile uint8_t *) rdData;

            /* Set end of transaction flag: Stop or Halt (following ReStart) */
            `$INSTANCE_NAME`_mstrControl = mode;

            /* Clear read status history */
            `$INSTANCE_NAME`_mstrStatus &= (uint8_t) ~`$INSTANCE_NAME`_MSTAT_RD_CMPLT;
      ...
      `=ReentrantKeil($INSTANCE_NAME . "_MasterSendStart")`
{
    ...
    if(`$INSTANCE_NAME`_SM_IDLE == `$INSTANCE_NAME`_state)
    {
        ...
        if(`$INSTANCE_NAME`_CHECK_BUS_FREE(`$INSTANCE_NAME`_MCSR_REG))
        {
            ...
            slaveAddress = (uint8_t) (slaveAddress << `$INSTANCE_NAME`_SLAVE_ADDR_SHIFT);
            if(0u != R_nW)
            {
