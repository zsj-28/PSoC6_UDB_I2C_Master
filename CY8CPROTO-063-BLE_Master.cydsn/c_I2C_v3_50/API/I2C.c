...
uint8_t `$INSTANCE_NAME`_initVar = 0u; /* Defines if component was initialized */

volatile uint8_t `$INSTANCE_NAME`_state;  /* Current state of I2C FSM */

extern const cy_stc_sysint_t `$INSTANCE_NAME`_I2C_IRQ_cfg;
...
void `$INSTANCE_NAME`_Init(void) `=ReentrantKeil($INSTANCE_NAME . "_Init")`
{
    ...
    NVIC_DisableIRQ    (`$INSTANCE_NAME`_ISR_NUMBER);
    //CyIntSetPriority(`$INSTANCE_NAME`_ISR_NUMBER, `$INSTANCE_NAME`_ISR_PRIORITY);
    #if (CY_CPU_CORTEX_M0P)
        #if (!defined(`$INSTANCE_NAME`_I2C_IRQ__INTC_CORTEXM0P_PRIORITY))
            #error `$INSTANCE_NAME`_I2C_IRQ not enabled on CM0 core, use the Interrupts Configuration to enable it.
        #else
            NVIC_SetPriority(`$INSTANCE_NAME`_ISR_NUMBER, `$INSTANCE_NAME`_I2C_IRQ__INTC_CORTEXM0P_PRIORITY);
        #endif
    #endif
    #if (CY_CPU_CORTEX_M4)
        #if (!defined(`$INSTANCE_NAME`_I2C_IRQ__INTC_CORTEXM4_PRIORITY))
            #error `$INSTANCE_NAME`_I2C_IRQ not enabled on CM4 core, use the Interrupts Configuration to enable it.
        #else
            NVIC_SetPriority(`$INSTANCE_NAME`_ISR_NUMBER, `$INSTANCE_NAME`_I2C_IRQ__INTC_CORTEXM4_PRIORITY);
        #endif
    #endif
    #if (`$INSTANCE_NAME`_INTERN_I2C_INTR_HANDLER)
        (void) Cy_SysInt_SetVector(`$INSTANCE_NAME`_ISR_NUMBER, &`$INSTANCE_NAME`_ISR);
...
void `$INSTANCE_NAME`_Stop(void) `=ReentrantKeil($INSTANCE_NAME . "_Stop")`
{
...
#if (`$INSTANCE_NAME`_FF_IMPLEMENTED)
    {
        ...
        /* Disable block */
        `$INSTANCE_NAME`_CFG_REG &= (uint8_t) ~`$INSTANCE_NAME`_CFG_EN_SLAVE;
        /* Wait for block reset before disable power */
        CyDelayCycles((uint32) blockResetCycles);

        /* Disable power to block */
        intState = CyEnterCriticalSection();
        `$INSTANCE_NAME`_ACT_PWRMGR_REG  &= (uint8_t) ~`$INSTANCE_NAME`_ACT_PWR_EN;
        `$INSTANCE_NAME`_STBY_PWRMGR_REG &= (uint8_t) ~`$INSTANCE_NAME`_STBY_PWR_EN;
        CyExitCriticalSection(intState);

        /* Enable block */
        `$INSTANCE_NAME`_CFG_REG |= (uint8_t) `$INSTANCE_NAME`_ENABLE_MS;
...
#if (`$INSTANCE_NAME`_MODE_SLAVE_ENABLED)
    {
        /* Disable bit counter */
        uint8_t intState = CyEnterCriticalSection();
        `$INSTANCE_NAME`_COUNTER_AUX_CTL_REG &= (uint8_t) ~`$INSTANCE_NAME`_CNT7_ENABLE;
        CyExitCriticalSection(intState);
    }
#endif /* (`$INSTANCE_NAME`_MODE_SLAVE_ENABLED) */
    ...
    /* Clear busy statuses */
#if (`$INSTANCE_NAME`_MODE_SLAVE_ENABLED)
    `$INSTANCE_NAME`_slStatus &= (uint8_t) ~(`$INSTANCE_NAME`_SSTAT_RD_BUSY | `$INSTANCE_NAME`_SSTAT_WR_BUSY);
#endif /* (`$INSTANCE_NAME`_MODE_SLAVE_ENABLED) */
}
