...
volatile uint8_t `$INSTANCE_NAME`_slStatus;   /* Slave Status  */

/* Transmit buffer variables */
volatile uint8_t * `$INSTANCE_NAME`_slRdBufPtr;   /* Pointer to Transmit buffer  */
volatile uint8_t   `$INSTANCE_NAME`_slRdBufSize;  /* Slave Transmit buffer size  */
volatile uint8_t   `$INSTANCE_NAME`_slRdBufIndex; /* Slave Transmit buffer Index */

/* Receive buffer variables */
volatile uint8_t * `$INSTANCE_NAME`_slWrBufPtr;   /* Pointer to Receive buffer  */
volatile uint8_t   `$INSTANCE_NAME`_slWrBufSize;  /* Slave Receive buffer size  */
volatile uint8_t   `$INSTANCE_NAME`_slWrBufIndex; /* Slave Receive buffer Index */

#if (`$INSTANCE_NAME`_SW_ADRR_DECODE)
    volatile uint8_t `$INSTANCE_NAME`_slAddress;  /* Software address variable */
...
uint8_t `$INSTANCE_NAME`_SlaveClearReadStatus(void) `=ReentrantKeil($INSTANCE_NAME . "_SlaveClearReadStatus")`
{
    ...
    `$INSTANCE_NAME`_slStatus &= (uint8_t) ~`$INSTANCE_NAME`_SSTAT_RD_CLEAR;
...
uint8_t `$INSTANCE_NAME`_SlaveClearWriteStatus(void) `=ReentrantKeil($INSTANCE_NAME . "_SlaveClearWriteStatus")`
{
    ...
    `$INSTANCE_NAME`_slStatus &= (uint8_t) ~`$INSTANCE_NAME`_SSTAT_WR_CLEAR;
     ...
     `=ReentrantKeil($INSTANCE_NAME . "_SlaveInitReadBuf")`
{
    if (NULL != rdBuf)
    {
        ...
        /* Set buffer pointer */
        `$INSTANCE_NAME`_slRdBufPtr   = (volatile uint8_t *) rdBuf;
     ...
     `=ReentrantKeil($INSTANCE_NAME . "_SlaveInitWriteBuf")`
{
    if (NULL != wrBuf)
    {
        `$INSTANCE_NAME`_DisableInt(); /* Lock from interrupt */
         ...
         /* Set buffer pointer */
        `$INSTANCE_NAME`_slWrBufPtr   = (volatile uint8_t *) wrBuf;
