...
`$INSTANCE_NAME`_BACKUP_STRUCT `$INSTANCE_NAME`_backup =
{
    ...
    volatile uint8_t `$INSTANCE_NAME`_wakeupSource;
...
#if (`$INSTANCE_NAME`_WAKEUP_ENABLED)
    /* Disable master */
    `$INSTANCE_NAME`_CFG_REG &= (uint8_t) ~`$INSTANCE_NAME`_ENABLE_MASTER;
...
#else
    /* Store enable state */
    `$INSTANCE_NAME`_backup.enableState = (uint8_t) `$INSTANCE_NAME`_IS_ENABLED;
...
void `$INSTANCE_NAME`_RestoreConfig(void) `=ReentrantKeil($INSTANCE_NAME . "_RestoreConfig")`
{
    ...
    /* Low power mode was Sleep - backup regulator is enabled */
    {
        ...
        intState = CyEnterCriticalSection();
        `$INSTANCE_NAME`_PWRSYS_CR1_REG &= (uint8_t) ~`$INSTANCE_NAME`_PWRSYS_CR1_I2C_REG_BACKUP;
        CyExitCriticalSection(intState);
