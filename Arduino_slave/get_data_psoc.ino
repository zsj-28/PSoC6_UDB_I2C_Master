...
byte aes_key[16] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0xFF, 0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0xAA, 0xBB, 0xCC, 0xDD};
...
void UART_receive() {
    while (Serial1.available() > 0) {
        ...
        if ((buffer_index == UART_buffer[1] + 3) && (buffer_index > 1)) {
            ...
            uint8_t decrypted_packet[16];
            aesLib.decrypt(decrypted_packet, UART_buffer, aes_key, 128);

            // Parse opcode and length
            opCode = decrypted_packet[0];
            dataLength = decrypted_packet[1];
            receivedCRC = decrypted_packet[dataLength + 2];

            // Calculate CRC8
            calculatedCRC = calculateCRC8(opCode, dataLength, &decrypted_packet[2]);
            buffer_index = 0;
            ...
            if (receivedCRC == calculatedCRC) {
                ...
                for (uint8_t i = 0; i < dataLength; i++) {
                    Serial.print("0x");
                    if (decrypted_packet[i + 2] < 0x10) {
                        Serial.print("0");
                    }
                    Serial.print(decrypted_packet[i + 2], HEX);
                    Serial.print(" ");
                }
                Serial.println();
            } else {
